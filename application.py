...
def buy():
    ...
    if request.method == "POST":
        ...
        user_id = session["user_id"]
        cash1 = db.execute("SELECT cash FROM users WHERE id = ?", user_id)[0]["cash"]

        if cash1 < transaction_value:
            return apology("Not enough money!")

        uptd_cash = cash1 - transaction_value
...
def history():
    ...
    user_id = session["user_id"]
    transactions = db.execute("SELECT symbol, price, shares, date, name FROM transactions WHERE user_id = ?", user_id)
...
def sell():
    ...
    if request.method == "GET":
        ...
        user_shares = db.execute("SELECT SUM(shares) as tshare FROM transactions WHERE user_id = ? AND symbol = ?", user_id, symbol)[
            0]["tshare"]
        ...
        db.execute("INSERT INTO transactions (user_id, symbol, shares, price, date, name) VALUES (?, ?, ?, ?, ?, ?)",
                   user_id, stock["symbol"], (-1)*shares, stock["price"], date, stock["name"])
